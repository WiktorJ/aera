...
from aera.autonomous.envs.jaw_geometry import DEFAULT_GRASP_SQUEEZE, engage_qpos
...
class GraspEngageConfig:
    ...
    Attributes:
        ...
        close_squeeze: How far past first pad contact the commanded jaw target
            (engage's ``close_ctrl_target``) must go to count as a committed
            close on the candidate. First contact itself comes from the pad
            geometry via :mod:`aera.autonomous.envs.jaw_geometry`, shared with
            the scripted expert's close target, so demo and gate agree by
            construction. Default 0.0006 m is the measured margin between first
            contact (-11.6 mm on a 24 mm block) and reliable engagement
            (-11.0 mm) — a close is tolerance-limited and stops slightly short
            of whatever it asks for. (Was ``close_depth_tol``, a *slack below*
            ``-pinch_half_width``: it inherited the same wrong contact model as
            the expert, leaving a [-12.5, -11.0) mm dead band where an attempt
            physically cannot pinch but is not reported shallow. Renamed rather
            than redefined so a stale config fails loudly.)
    ...
    close_squeeze: float = DEFAULT_GRASP_SQUEEZE  # past first pad contact
...
class KinematicGraspLock:
    ...
    def engage(
        ...
        units. When given, the candidate only locks if the command closes far
        enough to grip it — ``close_ctrl_target >= engage_qpos(half_width)``,
        i.e. past first pad contact by ``close_squeeze`` — so the policy has
        committed to a genuine close ON THIS object, not a twitch off
        full-open. The bound is per-candidate because a width-dependent close
        command scales with block width, so no flat threshold fits every size.
        (A gripper-binarized policy commands 0, full close, which clears the
        gate for every graspable preset by construction.) None (collection)
        skips the gate: there the scripted close has already physically
        happened.
        """
        ...
        # Close-depth gate: the commanded jaw target must close past this
        # candidate's first pad contact (by close_squeeze) before the weld is
        # granted. Both bounds come from the shared jaw geometry.
        if close_ctrl_target is not None:
            half_width = self._pinch_half_width(best_name)
            if half_width is not None and close_ctrl_target < engage_qpos(
                self.model, half_width, squeeze=cfg.close_squeeze
            ):
                return None, best_name, best_dist
