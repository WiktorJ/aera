...
# Block presets the gripper can reliably close on. The jaw opens to ~28.8mm inner
# clear; 24mm full width (half 0.012) is the validated max with descent
# clearance. Only the grasp target (object0) is restricted to these — distractors
# aren't grasped and may use any preset (larger sizes add visual variety).
_MAX_GRASPABLE_BLOCK_HALF = 0.012
_GRASPABLE_BLOCK_PRESETS = [
    i for i, (_edge, half) in enumerate(PLA_BLOCK_PRESETS)
    if half <= _MAX_GRASPABLE_BLOCK_HALF + 1e-9
]
...
# filament-realistic color, an FDM layer-line texture, and a coherent plastic
# finish. The renderer will never be photoreal, so we lean hard on *diversity*:
# a broad color palette and a wide, multi-modal finish distribution so the real
# object's appearance lands somewhere inside the training distribution.
...
# Mean brightness of the layer-line textures (== BASE_LEVEL in the generator
# script). The texture multiplies the rgba tint, so we pre-divide the tint by
# this to keep the final filament color at its intended value instead of ~12%
# dark. Keep in sync with scripts/generate_pla_textures.py.
_PLA_TEXTURE_MEAN = 0.88

# Filament color palette. Each key is a common English color word so it can be
# dropped straight into the policy prompt ("pick the {name} block"). The palette
# is intentionally COMPACT and the names are well separated — in hue, or in value
# for the look-alikes (brown is just a dark orange; pink a pale magenta) — so any
# two distinct names render as clearly different colors. That means in-scene
# separation needs no similarity check: drawing distinct names is enough, which
# keeps the color-conditioned task unambiguous.
#
# Diversity comes from WIDE per-name S/V ranges instead of more names: a muted
# dark green and a vivid bright green are both unmistakably "green", so the same
# label spans a lot of appearance variation across episodes. Ranges are kept
# inside what still reads as the name (e.g. yellow stays bright — a dark yellow
# would look olive). Chromatic entries are
# ("chroma", hue_center, hue_jitter, sat_range, val_range); near-neutral
# filaments are ("neutral", sat_range, val_range), value-driven.
_PLA_PALETTE = {
    "red":     ("chroma", 0.000, 0.010, (0.60, 1.00), (0.55, 0.92)),
    "orange":  ("chroma", 0.075, 0.010, (0.80, 1.00), (0.60, 0.95)),
    "brown":   ("chroma", 0.060, 0.012, (0.55, 0.85), (0.28, 0.45)),
    "yellow":  ("chroma", 0.135, 0.010, (0.65, 1.00), (0.75, 0.97)),
    "green":   ("chroma", 0.330, 0.030, (0.45, 1.00), (0.35, 0.88)),
    "cyan":    ("chroma", 0.500, 0.020, (0.45, 0.95), (0.60, 0.92)),
    "blue":    ("chroma", 0.610, 0.022, (0.60, 1.00), (0.45, 0.90)),
    "purple":  ("chroma", 0.755, 0.018, (0.50, 0.90), (0.45, 0.82)),
    "magenta": ("chroma", 0.880, 0.018, (0.70, 1.00), (0.50, 0.88)),
    "pink":    ("chroma", 0.930, 0.025, (0.22, 0.45), (0.82, 0.98)),
    "white":   ("neutral", (0.00, 0.05), (0.85, 0.98)),
    "gray":    ("neutral", (0.00, 0.06), (0.45, 0.68)),
    "black":   ("neutral", (0.00, 0.10), (0.04, 0.14)),
}

# Backwards-compatible alias: some call sites / docs referred to NAMED_COLORS.
NAMED_COLORS = _PLA_PALETTE

# Coherent finish presets: (probability, specular_range, shininess_range,
# reflectance_range). Sampling a finish *type* per part — rather than three
# independent uniforms — keeps each block reading as one consistent material (a
# matte print vs. a glossy one) instead of a muddy average, and lets us push the
# glossy tail far enough to look like polished / coated PLA in some renders. PLA
# is a dielectric, so even "glossy" keeps reflectance modest.
_BLOCK_FINISHES = (
    (0.40, (0.03, 0.18), (0.05, 0.25), (0.00, 0.00)),  # matte / textured filament
    (0.40, (0.18, 0.45), (0.25, 0.55), (0.00, 0.03)),  # satin — typical PLA
    (0.20, (0.45, 0.85), (0.55, 0.95), (0.03, 0.12)),  # glossy / coated
)
# Printed plates read flatter (top face off the build plate) — skew matte and
# cap the gloss lower than the blocks.
_PLATE_FINISHES = (
    (0.50, (0.03, 0.16), (0.05, 0.22), (0.00, 0.00)),
    (0.38, (0.16, 0.38), (0.22, 0.48), (0.00, 0.02)),
    (0.12, (0.38, 0.65), (0.45, 0.75), (0.02, 0.08)),
)


def _sample_distinct_color_names(n: int) -> list:
    """Draw n distinct palette color names for one scene (object, target, and
    their distractors). Because the palette names are mutually well separated,
    distinct names ⇒ visually distinct parts — no similarity check needed."""
    return list(np.random.choice(list(_PLA_PALETTE.keys()), n, replace=False))


def _pla_filament_rgba(color_name: str) -> Tuple[float, float, float, float]:
    """Map a palette color name to a 3D-PLA-filament-plausible opaque rgba.

    Real PLA is never a pure (1,0,0)-style primary: it's saturated-but-not-maxed
    and varies shot to shot. We sample S/V (and jitter the hue) inside the
    bucket so repeated 'red' blocks aren't identical, while staying well inside
    the color's name so the language prompt stays truthful.
    """
    spec = _PLA_PALETTE[color_name]
    if spec[0] == "neutral":
        _, s_range, v_range = spec
        h = float(np.random.uniform(0.0, 1.0))  # irrelevant at ~0 saturation
        s = float(np.random.uniform(*s_range))
        v = float(np.random.uniform(*v_range))
    else:
        _, h_center, h_jit, s_range, v_range = spec
        h = (h_center + float(np.random.uniform(-h_jit, h_jit))) % 1.0
        s = float(np.random.uniform(*s_range))
        v = float(np.random.uniform(*v_range))
    r, g, b = colorsys.hsv_to_rgb(h, s, v)
    return (float(r), float(g), float(b), 1.0)
...
def _sample_finish(finishes) -> Tuple[float, float, float]:
    """Pick one coherent (specular, shininess, reflectance) from a finish-preset
    distribution (matte / satin / glossy)."""
    r = np.random.random()
    cum = 0.0
    for p, spec_r, shin_r, refl_r in finishes:
        cum += p
        if r < cum:
            break
    else:
        _, spec_r, shin_r, refl_r = finishes[-1]
    return (
        float(np.random.uniform(*spec_r)),
        float(np.random.uniform(*shin_r)),
        float(np.random.uniform(*refl_r)),
    )
...
def _sample_pla_object_material(
    ...
    Wide DR around the PLA look: a filament base color sampled from the named
    bucket's (wide) S/V range, a layer-line texture applied most (but not all) of
    the time so the policy also sees clean prints, and a coherent matte/satin/
    glossy finish. The grayscale texture is tinted by `rgba` via MuJoCo's
    texture*rgba multiply, so one texture serves every color; the tint is
    pre-brightened by `_PLA_TEXTURE_MEAN` so the multiply lands on the intended
    filament value. Plates use a denser line pitch and a flatter finish (bigger,
    build-plate-smooth surface vs. the ~24mm cubes).
    """
    rgba = _pla_filament_rgba(color_name)
    rep_r = (1.5, 3.0) if is_plate else (1.0, 2.4)
    texture_name = None
    texrepeat = None
    if np.random.random() < 0.70:
        ...
        r, g, b, a = rgba
        rgba = (
            min(r / _PLA_TEXTURE_MEAN, 1.0),
            min(g / _PLA_TEXTURE_MEAN, 1.0),
            min(b / _PLA_TEXTURE_MEAN, 1.0),
            a,
        )
    specular, shininess, reflectance = _sample_finish(
        _PLATE_FINISHES if is_plate else _BLOCK_FINISHES
    )
    return MaterialConfig(
        texture_name=texture_name,
        rgba=rgba,
...
def generate_random_domain_rand_config(
    ...
    """
    # --- Color Selection ---
    # 6 distinct, well-separated palette names (object, target, and their
    # distractors). Distinct names ⇒ visually distinct parts, so the color-
    # conditioned task is unambiguous; each name's wide S/V range supplies the
    # episode-to-episode variation. Names feed the policy prompt.
    (
        object_color_name,
        target_color_name,
    ...
    ) = _sample_distinct_color_names(6)
    ...
    # size is intentionally NOT randomized: the visual is now a chamfered mesh
    # (assets/pla_block.obj) and MuJoCo can't scale a mesh geom at runtime, so the
    # block keeps a fixed geometric size (config.object_size, matching the mesh
    # scale in ar4_mk3.xml) and the rendered shape always agrees with the
    # collision box. The real PLA block is a single known size anyway. Mass /
    # friction / damping are still randomized.
    ...
    # --- Block visual preset (edge shape + size) ---
    # Each block independently picks one pre-baked (edge, size) preset; the
    # runtime shows that mesh and scales the collision box to match. The chamfer/
    # fillet shape is visual-only; the size also resizes the collision box.
    # object0 is the grasp target, so it's restricted to presets the gripper can
    # actually close on (measured open inner width ~28.8mm; 24mm — half 0.012 —
    # is the validated graspable max with descent clearance). The distractors
    # aren't grasped, so they use the full size range for visual variety.
    n_block_presets = len(PLA_BLOCK_PRESETS)
    object_block_variant = int(np.random.choice(_GRASPABLE_BLOCK_PRESETS))
    object_distractor1_block_variant = int(np.random.randint(n_block_presets))
    object_distractor2_block_variant = int(np.random.randint(n_block_presets))
    ...
    domain_rand_config = DomainRandConfig(
        ...
        object_block_variant=object_block_variant,
        object_distractor1_block_variant=object_distractor1_block_variant,
        object_distractor2_block_variant=object_distractor2_block_variant,
