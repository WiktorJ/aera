...
class TestTrajectoryDataCollector(unittest.TestCase):
    ...
    def _create_mock_image(self, timestamp_sec=10.0, timestamp_nanosec=0, encoding="bgr8", width=640, height=480):
    ...
    def test_episode_lifecycle(self):
        ...
        self.assertEqual(self.collector.current_episode_data["input_message"], input_message)
        self.assertIn("start_time", self.collector.current_episode_data)
        
        # Test stopping an episode
        with patch.object(self.collector, 'save_episode_data', return_value="test_file.json"):
    ...
    def test_start_episode_while_collecting(self):
        ...
        self.assertEqual(self.collector.current_episode_data["input_message"], "second episode")
    ...
    @patch('cv2.imencode')
    def test_record_joint_state(self, mock_imencode):
        ...
        self.assertEqual(len(recorded_data["arm_joint_positions"]), len(self.arm_joint_names))
        self.assertEqual(len(recorded_data["gripper_joint_positions"]), len(self.gripper_joint_names))
    ...
    def test_record_rgb_image(self, mock_imencode, mock_cv_bridge):
        ...
        mock_bridge_instance.imgmsg_to_cv2.return_value = np.zeros((480, 640, 3), dtype=np.uint8)
    ...
    def test_record_depth_image(self, mock_imencode, mock_cv_bridge):
        ...
        # Mock CvBridge
        mock_bridge_instance = Mock()
        mock_bridge_instance.imgmsg_to_cv2.return_value = np.zeros((480, 640), dtype=np.uint16)
        mock_cv_bridge.return_value = mock_bridge_instance
    ...
    def test_find_closest_in_buffer(self):
        ...
        # Test edge cases
        result = self.collector._find_closest_in_buffer(5.0, buffer, "test")  # Before first
        self.assertIsNone(result)
    ...
    def test_find_closest_in_buffer_statistics(self):
        ...
        self.assertIsNotNone(result)
        self.assertEqual(len(self.collector.sync_stats["rgb_discrepancies"]), initial_rgb_count + 1)
        self.assertAlmostEqual(self.collector.sync_stats["rgb_discrepancies"][-1], 0.02, places=3)
        ...
        self.assertIsNone(result)
        self.assertEqual(self.collector.sync_stats["failed_syncs"]["depth"], initial_fail_count + 1)
    ...
    def test_calculate_cartesian_velocity(self):
        ...
        velocity = self.collector._calculate_cartesian_velocity(current_pose, next_pose, dt)
    ...
    def test_calculate_cartesian_velocity_zero_dt(self):
        ...
        velocity = self.collector._calculate_cartesian_velocity(current_pose, current_pose, 0.0)
    ...
    def test_summarize_trajectory_data(self):
        ...
        self.assertGreater(summary["movement_analysis"]["total_cartesian_distance_meters"], 0)
        self.assertEqual(len(summary["movement_analysis"]["joint_movement_ranges_radians"]), 3)
        
        # Check manipulation analysis
        self.assertEqual(summary["manipulation_analysis"]["gripper_state_changes"], 1)
        self.assertEqual(summary["manipulation_analysis"]["episode_boundaries"]["first_points"], 1)
        self.assertEqual(summary["manipulation_analysis"]["episode_boundaries"]["last_points"], 1)
    ...
    def test_log_trajectory_summary(self):
        ...
        with patch.object(self.collector, 'summarize_trajectory_data', return_value=mock_summary):
    ...
    def test_is_first_and_last_in_prompt_group(self):
        ...
        prompt_groups = {
            "prompt_a": [10.0, 11.0, 14.0],
            "prompt_b": [12.0, 13.0]
        }
        
        # Test first in group
        self.assertTrue(self.collector._is_first_in_prompt_group(10.0, "prompt_a", prompt_groups))
        self.assertFalse(self.collector._is_first_in_prompt_group(11.0, "prompt_a", prompt_groups))
        
        # Test last in group
        self.assertTrue(self.collector._is_last_in_prompt_group(14.0, "prompt_a", prompt_groups))
        self.assertFalse(self.collector._is_last_in_prompt_group(11.0, "prompt_a", prompt_groups))
        
        # Test with non-existent prompt
        self.assertFalse(self.collector._is_first_in_prompt_group(10.0, "nonexistent", prompt_groups))
        self.assertFalse(self.collector._is_last_in_prompt_group(10.0, "nonexistent", prompt_groups))
